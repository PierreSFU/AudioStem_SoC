...
static int BurstReadWrite(char *FileRead, u32 DestinationBuffer, char *FileWrite, u32 size, u32 SourceAddress, u32 file_burst_size)
{
	...
	// Check if any errors occurred when opening
		if (rc) { // reading
...
//	// Reads header file -- FIX THIS, LET THIS HAPPEN!!!
//		// Copy data to buffer
...
//	// - End Reads header file
	...
	for(int i = 0; i < total_full_size_writes; i++) {
		...
		// Read header, top 44 bytes
		if(headers_not_copied) {
			int temp = 0;
			for(total_copied; total_copied < header_size; total_copied++){
				headerBuffer[total_copied] = burstBuffer[temp++];
			}

			// Copy data to buffer
			if(br >= header_size) {
				char* bufferFile = headerBuffer;
				memcpy(&wave_header, headerBuffer, header_size);
				print_data(&wave_header, file_size);
